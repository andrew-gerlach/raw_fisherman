...
select_clin_dat <- clin_dat %>% select(subject, vcis, line, mfigcopy, listtotal, storytotal_z, dspan_z, imis, diffss, cwi3cssfinal, cwi4cssfinal, coding_z)
...
# Calculate the average executive function measure
select_clin_dat <- select_clin_dat %>% group_by(subject) %>%
  mutate(
    avg_EF = mean(c(diffss, cwi3cssfinal, cwi4cssfinal), na.rm=TRUE)
  ) %>% ungroup()
# Transform variables that need to be
select_clin_dat <- select_clin_dat %>% mutate(
  #ln_line_z = log(line_z),
  #ln_mfigcopy_z = log(mfigcopy_z)
  adj_line = max(line, na.rm = TRUE) + 1 - line,
  ln_adj_line = log(adj_line),
  #sqrt_adj_line = sqrt(adj_line),
  #cubrt_adj_line = adj_line^(1/3),
  #inv_adj_line = adj_line^(-1),
  #inv_line = line^(-1),
  adj_mfigcopy = max(mfigcopy, na.rm=TRUE) + 1 - mfigcopy,
  ln_adj_mfigcopy = log(adj_mfigcopy),
  #sqrt_adj_mfigcopy = sqrt(adj_mfigcopy),
  #cubrt_adj_mfigcopy = adj_mfigcopy^(1/3),
  #inv_adj_mfigcopy = adj_mfigcopy^(-1),
  #inv_mfigcopy = mfigcopy^(-1)
)

# Create histograms to check the data
p1  <- ggplot(select_clin_dat, aes(x = vcis)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18) 
p2  <- ggplot(select_clin_dat, aes(x = ln_adj_line)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p3  <- ggplot(select_clin_dat, aes(x = ln_adj_mfigcopy)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
grid.arrange(p1, p2, p3, ncol = 2, top = textGrob("Visuospatial Measures", gp = gpar(fontsize = 20, fontface = "bold"))) # line and mfigcopy have prominent ceiling effects

p4  <- ggplot(select_clin_dat, aes(x = listtotal)) +  geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p5  <- ggplot(select_clin_dat, aes(x = storytotal_z)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p6  <- ggplot(select_clin_dat, aes(x = dspan_z)) +  geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p7  <- ggplot(select_clin_dat, aes(x = imis)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
grid.arrange(p4, p5, p6, p7, ncol = 2, top = textGrob("Working Memory Measures", gp = gpar(fontsize = 20, fontface = "bold")))

p8  <- ggplot(select_clin_dat, aes(x = diffss)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p9  <- ggplot(select_clin_dat, aes(x = cwi3cssfinal)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p10 <- ggplot(select_clin_dat, aes(x = cwi4cssfinal)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p11 <- ggplot(select_clin_dat, aes(x = avg_EF)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
grid.arrange(p8, p9, p10, p11, ncol = 2, top = textGrob("Executive Function Measures", gp = gpar(fontsize = 20, fontface = "bold")))

p12 <- ggplot(select_clin_dat, aes(x = dspan_z)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
p13 <- ggplot(select_clin_dat, aes(x = coding_z)) + geom_histogram(fill = "gray", color = "black", bins = 20) + theme_minimal(base_size = 18)
grid.arrange(p12, p13, ncol = 1, top = textGrob("Attention Measures", gp = gpar(fontsize = 20, fontface = "bold")))
...
```{r cognitive_data_correlations, include=T, fig.width=12, fig.height=10}
# Test broadly for correlations
library(corrplot)
library(RColorBrewer)

#dat_cog <- dat %>% select(edu.x,prior_strength)
dat_age_prior <- dat %>% select(subject,age,edu,dev,prior_strength) #don't think we can use prior strength here
...
dat3_cog <- dat3_wage %>% 
  mutate(edu_sc = scale(edu),
         ...
         pswq_sc = scale(pswq),
         rsq_sc = scale(rsq),
         dev_sc = scale(dev), # subject level deviation from the bayesian model (measure of performance)
         prior_strength_sc = scale(prior_strength),
         prior_likelihood_diff_mem_sc = scale(prior_likelihood_diff_mem),
         vcis_sc = scale(vcis),
         ln_adj_line_sc = scale(ln_adj_line),
         ln_adj_mfigcopy_sc = scale(ln_adj_mfigcopy),
         listtotal_sc = scale(listtotal),
         storytotal_z_sc = scale(storytotal_z),
         dspan_z_sc = scale(dspan_z),
         imis_sc = scale(imis),
         diffss_sc = scale(diffss),
         cwi3cssfinal_sc = scale(cwi3cssfinal),
         cwi4cssfinal_sc = scale(cwi4cssfinal),
         avg_EF_sc = scale(avg_EF),
         coding_z_sc = scale(coding_z)
         ) %>%
  select(age_sc, edu_sc, pswq_sc, rsq_sc, dev_sc, 
         prior_strength, prior_likelihood_diff_mem, prior_strength_sc, prior_likelihood_diff_mem_sc, 
         vcis_sc, ln_adj_line_sc, ln_adj_mfigcopy_sc, listtotal_sc, storytotal_z_sc,
         dspan_z_sc, imis_sc, diffss_sc, cwi3cssfinal_sc, cwi4cssfinal_sc,
         avg_EF_sc, coding_z_sc)
...
corrplot(pearson_correlation_results$r, type="upper", col=brewer.pal(n=8, name="BuYlRd")) #"RdYlBu"
#corrplot(pearson_p_values, type="upper", col=brewer.pal(n=8, name="RdYlBu"))
```


```{r cognitive_regression, include=T}
#Multiple regression
prior_model <- lm(prior_strength_sc ~ vcis_sc + ln_adj_line_sc + ln_adj_mfigcopy_sc + 
                    listtotal_sc + storytotal_z_sc + dspan_z_sc + imis_sc +
                    diffss_sc + avg_EF_sc + coding_z_sc,
                  data = dat3_cog)
summary(prior_model)
