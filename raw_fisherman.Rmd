...
select_clin_dat <- clin_dat %>% select(subject, vcis, line_z, mfigcopy_z, listtotal, storytotal_z, dspan_z, imis, diffss, cwi3cssfinal, cwi4cssfinal, coding_z, wrat5read, wrat5ss)
...
select_clin_dat <- select_clin_dat %>% mutate (
  ...
  working_memory_index = rowMeans(cbind(listtotal_sc, storytotal_sc, dspan_sc), na.rm=TRUE), #take out imis_sc
...
dat3_cog_all <- dat3 %>%
  select(age_sc, edu_sc, pswq_sc, rsq_sc, dev_sc, 
         ...
         coding_sc, attention_index, attention_index_sc, wrat5read, wrat5ss)
...
dat3_cog_all_select <- dat3_cog_all %>% select(age_sc, edu_sc, pswq_sc, rsq_sc, dev_sc, 
                                               prior_strength_sc, prior_likelihood_diff_mem_sc,
                                               vcis_sc, line_sc, mfigcopy_sc, ln_adj_visuospatial_index_sc,
                                               listtotal_sc, storytotal_sc, dspan_sc, imis_sc, working_memory_index_sc,
                                               diffss_sc, cwi3cssfinal_sc, cwi4cssfinal_sc,
                                               executive_functioning_index_sc, coding_sc, attention_index_sc, wrat5read, wrat5ss)
...
perf_model <- lm(dev_full_abs ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                  executive_functioning_index_sc + attention_index_sc + scale(age) + scale(edu) +
                  sex, data = dat3)
summary(perf_model)
car::vif(perf_model) 

prior_model <- lm(prior_mean1 ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc + scale(age) + scale(edu) +
                    sex, data = dat3)
car::vif(prior_model)
summary(prior_model)

update_model <- lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc + scale(age) + scale(edu) +
                    sex, data = dat3)
...
perf_model2 <- lm(dev_full_rms ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc + scale(pswq) + scale(rsq) +
                   scale(age) + scale(edu) + sex,
                   data = dat3)
summary(perf_model2)

prior_model2 <- lm(prior_mean1 ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc + scale(pswq) + scale(rsq) +
                   scale(age) + scale(edu) + sex, data = dat3)
summary(prior_model2)

update_model2 <- lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc + scale(pswq) + scale(rsq) +
                   scale(age) + scale(edu) + sex, data = dat3)
...
mod_perf_cog1 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu"), scale) %>%
    lmer(dev ~ ln_adj_visuospatial_index_sc + age + edu + sex + (1 | subject), .)
mod_perf_cog2 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu"), scale) %>%
...
mod_prior_cognition1 <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu"), scale) %>%
    lmer(prior_strength ~ ln_adj_visuospatial_index_sc + age + edu + sex + (1 | subject), .)
...
#worry/rumination finding controlling for all cognitive factors
...
mod_updateStrengthMEM_visuospatial_clinical <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc + age + edu + sex + pswq + rsq, .)
summary(mod_updateStrengthMEM_visuospatial_clinical)

mod_updateStrengthMEM_cog1 <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu"), scale) %>%
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc + age + edu + sex, .)
...
mod_perf_cog <- dat %>%
    ...
    lmer(dev_signed ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
           executive_functioning_index_sc + attention_index_sc + age + edu + sex +  (1 | subject), .)
summary(mod_perf_cog)
#add_p_values_to_lmer(mod_perf_cog)
# individual measures
mod_perf_cog1 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu"), scale) %>%
    lmer(dev_signed ~ ln_adj_visuospatial_index_sc + age + edu + sex + (1 | subject), .)
mod_perf_cog2 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu"), scale) %>%
...
# visuospatial
eff_visuospatial_dev <- ggpredict(mod_perf_cog, terms = "ln_adj_visuospatial_index_sc") # marginal effect of visuospatial (scaled in model), on dev
g_dev_visuospatial <- ggplot(eff_visuospatial_dev, aes(x = x, y = predicted)) +
  geom_point(
    data = dat3,
    aes(x = ln_adj_visuospatial_index_sc, y = dev_sc),
    size = 3,
    color = "black"
  ) +
...
pswq_model <- lm(pswq ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc + scale(age) +
                   scale(edu) + sex, data = dat3)
summary(pswq_model)

rsq_model <- lm(rsq ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc + scale(age) +
                  scale(edu) + sex, data = dat3)
summary(rsq_model)
...
mod_perf_cog1w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ ln_adj_visuospatial_index_sc*pswq + age + edu + sex + (1 | subject), .)
mod_perf_cog1r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ ln_adj_visuospatial_index_sc*rsq + age + edu + sex + (1 | subject), .)
mod_perf_cog2w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
...
mod_prior_cognition1w <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ ln_adj_visuospatial_index_sc*pswq + age + edu + sex + (1 | subject), .)
mod_prior_cognition1r <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ ln_adj_visuospatial_index_sc*rsq + age + edu + sex +  (1 | subject), .)
...
mod_updateStrengthMEM_cog1w <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc*pswq + age + edu + sex, .)
mod_updateStrengthMEM_cog1r <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc*rsq + age + edu + sex, .)
...
mod_perf_signed_cog1w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    ...
    lmer(dev_signed ~ ln_adj_visuospatial_index_sc*pswq + age + edu + sex + (1 | subject), .)
mod_perf_signed_cog1r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ ln_adj_visuospatial_index_sc*rsq + age + edu + sex + (1 | subject), .)
