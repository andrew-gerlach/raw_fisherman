...
perf_model <- lm(dev_full_abs ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
                   executive_functioning_index_sc + attention_index_sc, data = dat3)
summary(perf_model)
car::vif(perf_model) 
...
mod_perf_cog <- dat %>%
    ...
    lmer(dev ~ ln_adj_visuospatial_index_sc + working_memory_index_sc + 
           executive_functioning_index_sc + attention_index_sc + age + edu +  (1 | subject), .)
...
mod_perf_cog1 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu"), scale) %>%
    lmer(dev ~ ln_adj_visuospatial_index_sc + age + edu + (1 | subject), .)
mod_perf_cog2 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu"), scale) %>%
    lmer(dev ~ working_memory_index_sc + age + edu +  (1 | subject), .)
mod_perf_cog3 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu"), scale) %>%
    lmer(dev ~ executive_functioning_index_sc + age + edu +  (1 | subject), .)
mod_perf_cog4 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu"), scale) %>%
    lmer(dev ~ attention_index_sc + age + edu +  (1 | subject), .)
...
mod_prior_cognition1 <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu"), scale) %>%
    lmer(prior_strength ~ ln_adj_visuospatial_index_sc + age + edu +  (1 | subject), .)
mod_prior_cognition2 <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu"), scale) %>%
    lmer(prior_strength ~ working_memory_index_sc + age + edu +  (1 | subject), .)
mod_prior_cognition3 <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu"), scale) %>%
    lmer(prior_strength ~ executive_functioning_index_sc + age + edu +  (1 | subject), .)
mod_prior_cognition4 <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu"), scale) %>%
    lmer(prior_strength ~ attention_index_sc + age + edu +  (1 | subject), .)
...
mod_updateStrengthMEM_cog <- dat3 %>%
    ...
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc + working_memory_index_sc +
         executive_functioning_index_sc + attention_index_sc + age + edu, .)
summary(mod_updateStrengthMEM_cog)

mod_updateStrengthMEM_cog1 <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu"), scale) %>%
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc + age + edu, .)
mod_updateStrengthMEM_cog1a <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu"), scale) %>%
    lm(prior_likelihood_diff_mem ~ visuospatial_index_sc + age + edu, .)
mod_updateStrengthMEM_cog2 <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu"), scale) %>%
    lm(prior_likelihood_diff_mem ~ working_memory_index_sc + age + edu, .)
mod_updateStrengthMEM_cog3 <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu"), scale) %>%
    lm(prior_likelihood_diff_mem ~ executive_functioning_index_sc + age + edu, .)
mod_updateStrengthMEM_cog4 <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu"), scale) %>%
    lm(prior_likelihood_diff_mem ~ attention_index_sc + age + edu, .)
...
mod_perf_cog1 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu"), scale) %>%
    lmer(dev_signed ~ ln_adj_visuospatial_index_sc + age + edu + (1 | subject), .)
mod_perf_cog2 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu"), scale) %>%
    lmer(dev_signed ~ working_memory_index_sc + age + edu +  (1 | subject), .)
mod_perf_cog3 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu"), scale) %>%
    lmer(dev_signed ~ executive_functioning_index_sc + age + edu +  (1 | subject), .)
mod_perf_cog4 <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu"), scale) %>%
    lmer(dev_signed ~ attention_index_sc + age + edu +  (1 | subject), .)
summary(mod_perf_cog1)
summary(mod_perf_cog2)
summary(mod_perf_cog3)
summary(mod_perf_cog4)
...
``` 
## Cognition-Clinical Assocations
...
mod_perf_cog1w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    ...
    lmer(dev ~ ln_adj_visuospatial_index_sc*pswq + age + edu + (1 | subject), .)
mod_perf_cog1r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ ln_adj_visuospatial_index_sc*rsq + age + edu + (1 | subject), .)
mod_perf_cog2w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ working_memory_index_sc*pswq + age + edu +  (1 | subject), .)
mod_perf_cog2r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ working_memory_index_sc*rsq + age + edu +  (1 | subject), .)
mod_perf_cog3w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ executive_functioning_index_sc*pswq + age + edu +  (1 | subject), .)
mod_perf_cog3r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ executive_functioning_index_sc*rsq + age + edu +  (1 | subject), .)
mod_perf_cog4w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ attention_index_sc*pswq + age + edu +  (1 | subject), .)
mod_perf_cog4r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev","age","edu","pswq","rsq"), scale) %>%
    lmer(dev ~ attention_index_sc*rsq + age + edu +  (1 | subject), .)
...
mod_prior_cognition1w <- dat %>%
    ...
    lmer(prior_strength ~ ln_adj_visuospatial_index_sc*pswq + age + edu + (1 | subject), .)
mod_prior_cognition1r <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ ln_adj_visuospatial_index_sc*rsq + age + edu + (1 | subject), .)
mod_prior_cognition2w <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ working_memory_index_sc*pswq + age + edu + (1 | subject), .)
mod_prior_cognition2r <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ working_memory_index_sc*rsq + age + edu + (1 | subject), .)
mod_prior_cognition3w <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ executive_functioning_index_sc*pswq + age + edu + (1 | subject), .)
mod_prior_cognition3r <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ executive_functioning_index_sc*rsq + age + edu + (1 | subject), .)
mod_prior_cognition4w <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ attention_index_sc*pswq + age + edu + (1 | subject), .)
mod_prior_cognition4r <- dat %>%
    filter(trialnum == 1, POS != 0) %>%
    mutate_at(c("prior_strength","age","edu","pswq","rsq"), scale) %>%
    lmer(prior_strength ~ attention_index_sc*rsq + age + edu + (1 | subject), .)
...
mod_updateStrengthMEM_cog1w <- dat3 %>%
    ...
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc*pswq + age + edu, .)
mod_updateStrengthMEM_cog1r <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ ln_adj_visuospatial_index_sc*rsq + age + edu, .)
mod_updateStrengthMEM_cog1aw <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ visuospatial_index_sc*pswq + age + edu, .)
mod_updateStrengthMEM_cog1ar <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ visuospatial_index_sc*rsq + age + edu, .)
mod_updateStrengthMEM_cog2w <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ working_memory_index_sc*pswq + age + edu, .)
mod_updateStrengthMEM_cog2r <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ working_memory_index_sc*rsq + age + edu, .)
mod_updateStrengthMEM_cog3w <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ executive_functioning_index_sc*pswq + age + edu, .)
mod_updateStrengthMEM_cog3r <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ executive_functioning_index_sc*rsq + age + edu, .)
mod_updateStrengthMEM_cog4w <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ attention_index_sc*pswq + age + edu, .)
mod_updateStrengthMEM_cog4r <- dat3 %>%
    filter(prior_likelihood_diff_mem > -0.75) %>%
    mutate_at(c("prior_likelihood_diff_mem","age","edu","pswq","rsq"), scale) %>%
    lm(prior_likelihood_diff_mem ~ attention_index_sc*rsq + age + edu, .)
...
mod_perf_signed_cog1w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    ...
    lmer(dev_signed ~ ln_adj_visuospatial_index_sc*pswq + age + edu + (1 | subject), .)
mod_perf_signed_cog1r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ ln_adj_visuospatial_index_sc*rsq + age + edu + (1 | subject), .)
mod_perf_signed_cog2w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ working_memory_index_sc*pswq + age + edu + (1 | subject), .)
mod_perf_signed_cog2r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ working_memory_index_sc*rsq + age + edu + (1 | subject), .)
mod_perf_signed_cog3w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ executive_functioning_index_sc*pswq + age + edu +(1 | subject), .)
mod_perf_signed_cog3r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ executive_functioning_index_sc*rsq + age + edu +(1 | subject), .)
mod_perf_signed_cog4w <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ attention_index_sc*pswq + age + edu + (1 | subject), .)
mod_perf_signed_cog4r <- dat %>%
  filter(blocknum %in% blocks, trialnum > 1) %>%
    mutate_at(c("dev_signed","age","edu","pswq","rsq"), scale) %>%
    lmer(dev_signed ~ attention_index_sc*rsq + age + edu + (1 | subject), .)
...
dev_formula_fn <- function(varname) {
  as.formula(paste("dev ~ ", varname, " + scale(age) + scale(edu) + (1 | subject)"))
}
dev_pswq_formula_fn <- function(varname) {
  as.formula(paste("dev ~ ", varname, "*scale(pswq) + scale(age) + scale(edu) + (1 | subject)"))
}
dev_rsq_formula_fn <- function(varname) {
  as.formula(paste("dev ~ ", varname, "*scale(rsq) + scale(age) + scale(edu) + (1 | subject)"))
}

prior_formula_fn <- function(varname) {
  as.formula(paste("prior_strength ~ ", varname, " + scale(age) + scale(edu) + (1 | subject)"))
}
prior_pswq_formula_fn <- function(varname) {
  as.formula(paste("prior_strength ~ ", varname, "*scale(pswq) + scale(age) + scale(edu) + (1 | subject)"))
}
prior_rsq_formula_fn <- function(varname) {
  as.formula(paste("prior_strength ~ ", varname, "*scale(rsq) + scale(age) + scale(edu) + (1 | subject)"))
}

update_formula_fn <- function(varname) {
  as.formula(paste("prior_likelihood_diff_mem ~ ", varname, " + scale(age) + scale(edu)"))
}
update_pswq_formula_fn <- function(varname) {
  as.formula(paste("prior_likelihood_diff_mem ~ ", varname, "*scale(pswq) + scale(age) + scale(edu)"))
}
update_rsq_formula_fn <- function(varname) {
  as.formula(paste("prior_likelihood_diff_mem ~ ", varname, "*scale(rsq) + scale(age) + scale(edu)"))
}
...
for (v in vars) {
  ...
  # MLM for performance (deviation from optimal)
  disp("MLM, deviation from optimal")
  model1 <- lmer(dev_formula_fn(v), data = dat)
  summary(model1)
  disp("MLM deviation from optimal - worry interaction")
  model2 <- lmer(dev_pswq_formula_fn(v), data = dat)
  summary(model2)
  disp("MLM deviation from optimal - rumination interaction")
  model3 <- lmer(dev_rsq_formula_fn(v), data = dat)
  summary(model3)
  
  # MLM for prior strength 
  disp("MLM, prior strength")
  ...
  # Update strength regression model
  disp("linear model, update strength")
